...
class EmotionRecognition:
  ...
  def load_model(self):
    if isfile(join(SAVE_DIRECTORY, SAVE_MODEL_FILENAME+'.index')):
      ...
      # Make predictions for validation and test data
      y_valid_pred = np.argmax(self.model.predict(self.dataset.images_valid), axis=1)
      y_test_pred = np.argmax(self.model.predict(self.dataset.images_test), axis=1)
    
      # Print confusion matrix for validation data
      print("Confusion matrix for validation data:")
      print(confusion_matrix(np.argmax(self.dataset._labels_valid, axis=1), y_valid_pred))

      # Print confusion matrix for test data
      print("Confusion matrix for test data:")
      print(confusion_matrix(np.argmax(self.dataset.labels_test, axis=1), y_test_pred))
