...
class DecisionTree:
    ...
    def Entropy(self, Y):
        """Compute Entropy impurity of a non-empty node.
        """
        total_cases = Y.size
        return -sum((np.sum(Y == c) / total_cases) * (np.log2((np.sum(Y == c) / total_cases))) for c in range(self.n_classes))
    ...
    def calc_impurity(self, X, Y):
        ...
        best_EN = -(sum((n/ total_cases) * (n / total_cases) for n in num_parent))
        ...
        for col in range(self.n_features):
            ...
            """Loop through all possible values of the features"""
            for i in range(1, total_cases ):
                c = classes[i - 1]
                num_left[c] += 1
                num_right[c] -= 1
                
                # Calculating left and right child entropy
                EN_left_child= -sum((num_left[x] / i)*(np.log2(num_left[x] / i)) for x in range(self.n_classes))
                EN_right_child=-sum((num_right[x] / i)*(np.log2(num_right[x] / i)) for x in range(self.n_classes))

                """weighted average of entropy of the two childs"""
                ...
                """ Finding the lowest entropy and assining the index and corresponding threshold value for split"""
                if EN < best_EN:
                    best_EN = EN
                    best_col = col
    ...
    def learn(self, X, Y, depth=0):
        ...
        if depth < self.max_depth: 
            ...
            print("The rule-set for Node-",depth," Index no-",col," split value",thr)
            if col is not None:
                indices_left = X[:, col] < thr
                X_left, Y_left = X[indices_left], Y[indices_left]
                X_right, Y_right = X[~indices_left], Y[~indices_left]
                # Node values saved for classification (Index and split value)
                node.feature_index = col
                node.threshold = thr
                print("Data left to parse",len(Y_left),len(Y_right))
                # Receursion 
...
#    test_set=np.array(test_set).astype('float')
   ...
   # Classify the test set using the tree we just constructed
    ...
    f.write("accuracy:",round(statistics.mean(accuracy),4)*100)
    f.close()
